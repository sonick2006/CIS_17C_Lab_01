...
TEST(BookstoreTest, AddBook) {
    ...
    store.addBook(b);
    EXPECT_EQ(store.getBooks().size(), 1);
    EXPECT_EQ(store.getBooks()[0].title, "Test Book");
}
...
TEST(BookstoreTest, Checkout) {
    ...
    store.addToCart(b, 1);
    store.checkout(u);
    EXPECT_EQ(store.getOrders().size(), 1);
    EXPECT_EQ(store.getOrders()[0].orderId, "ORD1");
    EXPECT_DOUBLE_EQ(store.getOrders()[0].totalAmount, 10.0);
}
...
TEST(BookstoreTest, EmptyCartCheckout) {
    ...
    User u = {"U1", "User", "email", "pass", "addr"};
    store.checkout(u);
    EXPECT_EQ(store.getOrders().size(), 0);
}
...
TEST(BookstoreTest, MultipleOrders) {
    ...
    store.addToCart(b, 2);
    store.checkout(u2);
    EXPECT_EQ(store.getOrders().size(), 2);
    EXPECT_EQ(store.getOrders()[1].orderId, "ORD2");
    EXPECT_DOUBLE_EQ(store.getOrders()[1].totalAmount, 20.0);
}
